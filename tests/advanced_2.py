...
def preprocess_image(image_path):
    ...
    _, binary = cv2.threshold(gray, 150, 32, cv2.THRESH_BINARY)
...
def detect_text_regions(binary):
    ...
    min_area = 100
    max_aspect_ratio = 10  # Adjust as needed
    text_regions = []
    for c in contours:
...
def ocr_text(image, regions):
    ...
    for x, y, w, h in regions:
        ...
        text = pytesseract.image_to_string(roi, config='--psm 6 --oem 3 -c tessedit_char_whitelist=0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz:-|()/&,.')
...
def image_to_text_debug(image_path):
    ...
    ocr_results = ocr_text(binary, regions)
    
    return ocr_results

# Example usage
image_path = './CSE_5TH_SUM24-result_images\\648675-GUPTA KARN DEEPAK.png'
results = image_to_text_debug(image_path)

# C:\DevParapalli\Projects\RTMNU\result-scraping\CSE_5TH_SUM24-result_images\648675-GUPTA KARN DEEPAK.png
# Print results
print("\nOCR Results:")
